...
@section('title', 'SCOB ' . $year . ' - Appellate Division')

@section('content')
    <div class="container py-4">
        <h4 class="mb-4">SCOB Year {{ $year }} - Appellate Division</h4>
        ...
        <ul class="nav nav-tabs mb-4">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('subscriber.scobYear.appellate', $year) }}">Appellate Division</a>
            </li>
            <li class="nav-item">
                ...
                @foreach($appellateDecisions as $decision)
                    <div class="col d-flex">
                        <a class="card-item d-flex flex-column h-100 w-100 p-3 border rounded shadow-sm text-decoration-none"
                            href="{{ route('subscriber.singleDecision', [$decision->id, Crypt::encrypt('scobYearAppellate')]) }}">
                            ...
                            <p class="card-text clamp-4 flex-grow-1 text-secondary small">
                                {!! Str::limit(strip_tags($decision->judgment), 150) !!}
                            </p>
        ...
        @else
            <div class="text-center py-5">
                ...
                <p class="mt-3">No Appellate Division cases found for year {{ $year }}</p>
            </div>
        @endif
        ...
        <div class="mt-4">
            ...
            <select class="form-select w-auto d-inline-block ms-2"
                onchange="window.location.href='{{ route('subscriber.scobYear.appellate', '') }}/' + this.value">
